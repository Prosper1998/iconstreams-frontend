...
document.addEventListener('DOMContentLoaded', () => {
    loadContent(`${API_BASE_URL}/api/content?category=Featured`, 'featuredContainer', 'Featured Content');
    initVideoPlayer();
    ...
    searchInputs.forEach(input => {
        input?.addEventListener('input', async () => {
            ...
            try {
                const res = await fetch(`${API_BASE_URL}/api/content?category=Featured`);
                ...
                `).join('') || `<p class="empty-message">No results for "${query}" in Featured Content.</p>`;
            } catch (error) {
