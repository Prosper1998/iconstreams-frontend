...
document.addEventListener('DOMContentLoaded', async () => {
    const movieCategories = ['Drama', 'Thriller', 'Adventure', 'Comedy'];
    const query = movieCategories.map(cat => `category=${encodeURIComponent(cat)}`).join('&');
    loadContent(`${API_BASE_URL}/api/content?${query}`, 'moviesContainer', 'Movies');
    initVideoPlayer();
    ...
    searchInputs.forEach(input => {
        input?.addEventListener('input', async () => {
            ...
            try {
                ...
                const filtered = data
                    .filter(item => movieCategories.includes(item.category))
                ...
                `).join('') || `<p class="empty-message">No results for "${query}" in Movies.</p>`;
            } catch (error) {
